...
# PDF文件名标题匹配（核心方法）
python $SKILL_DIR/progress_manager.py fix-pdf-names
# → PDF文件名格式: "Author 等 - Year - Title.pdf"
# → 提取 Year 后面的标题，去文献库匹配文献标题
# → 匹配策略：精确标题 → 模糊标题(≥80%) → 作者+年份兜底
# → 生成 pdf_mapping.json（Key → 实际PDF文件名）
# → 扫描 pdfs/ 与文献池 CSV 匹配
# → 生成 pdf_mapping.json（Key → 实际文件名）
# → 列出无法自动匹配的给用户确认
...
| 17 | **PDF文件名纠偏** | 精确匹配pdfs/{Key}.pdf，大小写/空写差异导致匹配失败 | `fix-pdf-names`：模糊匹配（≥70%相似度自动记录），生成pdf_mapping.json，子agent查表 |
| 18 | **PDF文件名标题匹配** | 依赖文件名/作者+年份，无法处理同名作者不同论文 | 提取PDF文件名中"Year - Title"部分的标题，直接匹配文献库标题（≥80%相似度） |
