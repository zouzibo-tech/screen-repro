#!/usr/bin/env python3
"""
progress_manager.py — screen-repro v2.3 进度管理器
====================================================
...
def verify():
    ...
    # 新增：验证结果计数
    actual_counts = {"INCLUDE": 0, "EXCLUDE": 0, "MAYBE": 0}
    for sd in SUBDIRS:
        md_dir = RECORDS / sd
        if md_dir.exists():
            count = len(list(md_dir.glob("*.md")))
            if sd in actual_counts:
                actual_counts[sd] = count
    
    count_mismatch = False
    for decision, expected in d["results"].items():
        if decision in actual_counts:
            actual = actual_counts[decision]
            if actual != expected:
                count_mismatch = True
    ...
    for decision in ["INCLUDE", "EXCLUDE", "MAYBE"]:
        expected = d["results"].get(decision, 0)
        actual = actual_counts.get(decision, 0)
        status = "✅" if expected == actual else "❌"
        print(f"   {status} {decision}: 进度={expected}, 文件={actual}")
    
    if count_mismatch:
        print("\n⚠️ 结果计数不一致，将修正:")
        for decision in ["INCLUDE", "EXCLUDE", "MAYBE"]:
            d["results"][decision] = actual_counts.get(decision, 0)
        dirty = True

    if not missing and not unmarked and not count_mismatch:
        print("\n✅ 完全一致，无需修复")
